...
async function fetchSteamGames(): Promise<{ title: string; releaseDate: string; originalPrice: string | null; gameurl: string | null; gameid: string | null; imageUrl: string | null }[]> {
...
async function extractGameInfoFromHTML(htmlString: string): Promise<{ title: string; releaseDate: string; originalPrice: string | null; gameurl: string | null; gameid: string | null; imageUrl: string | null }[]> {
    ...
    async function fetchGameDetails(gameurl: string | null): Promise<{ imageUrl: string | null; freeUntil: string | null }> {
        ...
        try {
            ...
            const img = dom.window.document.querySelector('#gameHeaderImageCtn .game_header_image_full');
            const imageUrl = img ? img.getAttribute('src') : null;

            // Extract freeUntil date
            let freeUntil: string | null = null;
            const freeUntilP = dom.window.document.querySelector('p.game_purchase_discount_quantity');
            if (freeUntilP) {
    ...
    const gameInfoPromises = Array.from(gameDivs).map(async (gameDiv: Element) => {
